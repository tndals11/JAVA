...
* - repository: 데이터 저장소(DB)와의 상호 작용
* - dto : 계층 산 데이터 전송을 위한 객체
...
import org.example.z_project.phr_solution.dto.Health.record.request.RecordRequestDto;
    ...
    private static boolean processChoice(int choice) {
        switch (choice) {
            ...
            case 1: {
                ...
                System.out.println(requestDto.getName() + " 환자가 정상적으로 등록되었습니다.");
                break;
            }
            case 2: {
            ...
            case 5: {
                long id = InputHandler.getIdInput();
                PatientDetailResponseDto responseDto = patientController.getPatientById(id);
                patientController.deletePatient(id);
                System.out.println(responseDto.getName() + "님이 정삭적으로 삭제되었습니다.");
                break;
            }
            case 6: {
