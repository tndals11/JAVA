...
*  === 스트림(Stream, 흐름) API ===
* : 자바 8부터 도입된 기능
* , 컬렉션 또는 배열에 저장된 데이터를 선언형(직관, 간결) 방식으로 처리할 수 있께 지원하는 도구
* - 데이터 필터링, 매핑, 변환, 집계 등 다양한 작업 수행
* - 람다식과 결합하여 가독성 높은 코드 작성이 가능
*
* 1. 스트림 3단계 구조 (데이터 파이프라인)
...
//                .forEach(name -> System.out.println(name));
        ...
        names.stream()
                .distinct()
                .sorted()
                // 매개변수가 1개일경우 :: 을 붙여서 줄일 수 있다
                .forEach(System.out::println);
        
        // 4
        System.out.println("==== 총 인원 수 출력 ====");
        long count = names.stream().distinct().count(); // 중복제거 distinct
