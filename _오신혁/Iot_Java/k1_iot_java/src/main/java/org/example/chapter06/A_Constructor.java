...
class Example2 {
    ...
    Example2(int value) { // 매개변수를 가진 생성자
        this.value = value;
    }
...
class  Car {
    ...
    Car (String color){
        ...
        // door = 5;
        this(color, " auto ", 4);
        // 사용자 정의 생성자 내에서 타 생성자 호출 시
        // 반드시 첫 줄에서만 생성 가능!!
        // >> 호출된 생성자가 변수값을 초기화하기 때문에
        //     , 현재의 생성자가 수행하는 작업을 유지하려면 this() 생성자 뒤에 저장
        //     - 무의미함 방지
        door = 6;

        // this(color, "manual", 4);
        // 생성자에서는 타 생성자를 한 번만 호출 가능하다
    }
}
