...
@AllArgsConstructor // final이기 때문에
@Getter
@EqualsAndHashCode
class Address {
    // 데이터 식별 값 필요가 x (ID가 없음)
    // 동일성 보다 동등성이 중요 (VO는 값이 같으면 동일하다고 본다)
...
class UserDao {
    ...
    public UseResponseDto getUserById(String userId) {
        // DB에 접근해 해당 userId 사용자의 닉네임을 조회했다고 가정
        String id = "qwe";
        String nickname = "개구리";

        return new UseResponseDto(id, nickname);
    }
}
...
//@Table(name = "users") - DB의 users 테이블과 연결을 암시
class User {
    private Long id;
    private String userId;
    private String password;
