...
// >> 다양한 디자인 패턴(Design Patten)이 SOLID 설계 원칙에 따라 만들어짐
...
// : 소프트웨어 엔티티 (클래스, 모듀르 함수 등)는 확장에는 열려있어야 하고, 수정에는 닫혀있어야 한다.
class LuchMenu {
    void serveLuch(String studentType) {
...
// 2) 올바른 설계 예시 -
interface LuchmenuInterface {
    void serveLunch();
}

class RegularStudent implements LuchmenuInterface {
    @Override
...
class VegetableStudent implements LuchmenuInterface {
    @Override
    public void serveLunch() {
...
class AllergyStudent implements LuchmenuInterface {
    @Override
    public void serveLunch() {
...
// 1) 잘못된 설계 예시 - Student의 동작과 InjuredStudent가 같은 메서드를 통해 서로 다른 행동을 실행함 (같은 메서드를 통해)
class Student {
    void playSoccer() {
