...
public class UserServiceImpl implements UserService {
    ...
    public void registerUser(UserRegisterRequestDto dto) {
        ...
        if (!password.equals(passwordCheck)) {
            // 비밀번호와 비밀번호 확인이 일치하지 않음 - 회원가입 불가X
    ...
    public void login(UserLoginRequestDto dto) {
        ...
        // ifPresentOrElse(a, b);
        // : 정보가 존재하는 경우 a로직 실행, 존재하지 않을 경우 b로직 실행
        userRepository.findByUserId(userId).ifPresentOrElse(user -> {
            ...
            // ifPresentOrElse () -> 뒤로 빠진거
        }, () -> System.out.println("로그인 실패: 사용자 ID를 찾을 수 없습니다."));
    }

    @Override
    public void logout() {
        // loggedUser에는 로그인 상태에 값이 담겨져있다.
        if (loggedUser != null) {
            // 로그인 중
    ...
    public boolean isLoggedIn() {
        // loggedUser의 값이 null이 아니면(로그인 중이면) true를 반환
