...
>> 모듈의 독립성을 결합도(coupling)외 응집도(cohesion)의 기준 단계로 측정
...
2. 응집도(높은 응집도)
      : 하나의 클래스나 모듈 안에서 관련된 작업들끼리 얼마나 잘 모며 있는지를 나타냄
      >> 한 모듈 내의 구성요소들 연관성
...
class ClassRoom {
    ...
    // : 클래스가 직접 객체를 생성하지x, 외부에서 객체를 주입받도록 설계
...
// >> ClassRoom이 직접 객체를 생성x, 외부에서 주입 방식 (IoC 예시)
...
public class D_Coupling_Cohesion {
    public static void main(String[] args) {
        ...
        ClassRoom koreaClass = new ClassRoom(koreanTeacher);
