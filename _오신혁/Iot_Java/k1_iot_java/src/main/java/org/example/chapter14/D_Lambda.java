...
@FunctionalInterface  // 추상 메서드가 한 개만 존재해야한다
interface MyFunctionalInterface {
    void doSomething();
    //    void anotherSomething();
    // >> 함수형 인터페이스는 단 하나의 추상메서드만 존재해야한다
}
...
public class D_Lambda {
    public static void main(String[] args) {
        ...
        Calculator calculator2 = (a, b) -> {
            int result = a + b;
            return a + b; // 여러 줄 구현은 {} 생략 불가! (반환값도 반드시 return 사용)
        };
        ...
        StringLength stringLength = str -> str.length(); // 실행문이 한 줄: return + {} 생략가능
